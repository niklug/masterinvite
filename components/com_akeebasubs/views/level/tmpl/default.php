...
$template_short = JRequest::getVar('template_short');
if($template_short) {
    $short_class = 'class="template_short"';
} else {
    if($this->aid_exists == false) {
        echo '<a href="' . JURI::base() . '"> Back</a>' . "<br/>";
        die('no or wrong affiliate id');
    }
}
...
<form action="<?php echo JRoute::_('index.php?option=com_akeebasubs&view=subscribe&layout=default&slug='.$this->input->getString('slug',''))?>" method="post"
	...
	<div id="paymentmethod-container" <?php echo ($this->validation->price->gross < 0.01) ? 'style="display: none;"' : '' ?>>
        ...
        if($template_short) {
        ?>
            <button id="subscribenow" class="cyan" type="submit">
