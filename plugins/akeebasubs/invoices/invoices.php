...
class plgAkeebasubsInvoices extends JPlugin
{
	...
	{       
                //mail('npkorban@mail.ru', 'test', 'type ' . $info['type']);
		...
		if($whenToGenerate == 1) {
			...
			$specialCasePending = in_array($row->state, array('P','C', 'N')) && !$row->enabled;
		...
		if( $generateAnInvoice ) {
			$db = JFactory::getDBO();
                        ...
                        //mail('npkorban@mail.ru', 'test', $info['type'] . ' -  ' . $generateAnInvoice);
