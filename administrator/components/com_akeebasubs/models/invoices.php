...
class AkeebasubsModelInvoices extends FOFModel
{
	...
	public function createInvoice($sub)
	{
            ...
            // for credit nota
            if($sub->type  == 'creditnota') {
                $record_table = 'creditnotas';
            }
                
            //mail('npkorban@mail.ru', 'test', 'type ' . $sub->type);
            // for text in invoice
                $sub->paid = 'PAID';
              ...
              //mail('npkorban@mail.ru', 'state', print_r($sub, true));
          
              // not for credit nota
              if($sub->type != 'creditnota') {
                  if(($sub->state == 'C') && ($sub->paid == 'PAID'))  {
                                    // add credit if paid 
                      ...
                      $credit_value = $credit_model->addCredit($sub->user_id);
                      if ($credit_value) {
                        $billing_model->addCreditsToUserBallance($sub->akeebasubs_affiliate_id, $credit_value);
                      }
                  }
              }
		...
		$query = $db->getQuery(true)
			->select('*')
			->from($db->qn('#__akeebasubs_' . $record_table))
		...
		// Get the template
                ...
                // not for credit nota
                if($sub->type == 'creditnota') {
                    $template = $this->findTemplate('Creditnota');
                } else {
                    $template = $this->findTemplate($sub->akeebasubs_level_id);
                }
                //mail('npkorban@mail.ru', 'test', $sub->type  .$template);
			...
			'[VAT_NOTICE]'				=> $vat_notice,
                ...
                if($sub->type  == 'creditnota') {
                    $extras = array(
                        ...
                        '[CREDIT:TRANSACTIONS]' => $sub->transactions,
                        '[CREDIT:PAYMENT_METHOD]' => $sub->payment_method,
                        '[CREDIT:AMOUNT]' => round($sub->amount, 2),
                        '[CREDIT:AMOUNT_WITHOUT_TAX]' => round($sub->amount - ($sub->amount * ($sub->tax_percent / 100)), 2),
                        '[CREDIT:TAX_AMOUNT]' => round($sub->amount * ($sub->tax_percent / 100), 2),
		...
		if($existingRecord)
		{
			...
			$db->updateObject('#__akeebasubs_' . $record_table, $o, 'akeebasubs_subscription_id');
		}
		else
		{
			$o = (object)$invoiceData;
			$db->insertObject('#__akeebasubs_' . $record_table, $o);
		}
		...
		//$sub->save($updates);
                // for credit nota
                if($sub->type  == 'creditnota') {
                    $this->updateSubscriptionCreditnotaId($invoice_no, $sub->akeebasubs_subscription_id);
                } else {
                    $this->updateSubscriptionInvoiceId($invoice_no, $sub->akeebasubs_subscription_id);
                }
         ...
         */
        public function updateSubscriptionInvoiceId($invoice_id, $subscription_id) {
            $db = &JFactory::getDbo();
            $query = "UPDATE #__akeebasubs_subscriptions SET akeebasubs_invoice_id='$invoice_id' WHERE akeebasubs_subscription_id='$subscription_id'";
            $db->setQuery($query);
            $db->query();
        }
         ...
         * @param type $subscription_id
         */
        public function updateSubscriptionCreditnotaId($creditnota_id, $subscription_id) {
            $db = &JFactory::getDbo();
            $query = "UPDATE #__akeebasubs_subscriptions SET credit_invoice_number='$creditnota_id' WHERE akeebasubs_subscription_id='$subscription_id'";
            $db->setQuery($query);
            $db->query();
        }
	...
	private function findTemplate($level_id)
	{
		...
		if (!empty($templates))
		{
			...
			{
                                // for credit nota
                                if (($level_id == 'Creditnota') && ($template->title == 'Creditnota')) {
                                    $ret = $template->template;
                                    break;
                                } else if($template->title == 'Creditnota') {
                                    continue;
                                }
	 ...
	 */
	public function createPDF($record_table)
	{
		...
		$name = $hash . '_' . $invoiceRecord->invoice_no . '.pdf';
                ...
                if($record_table == 'creditnotas') {
                    $path = JPATH_ADMINISTRATOR . '/components/com_akeebasubs/creditnotas/';
                } else {
		...
		if ($ret)
		{
			...
			$db->updateObject('#__akeebasubs_' . $record_table, $invoiceRecord, 'akeebasubs_subscription_id');
	 ...
	 */
	public function emailPDF($record_table)
	{
		// Get the invoice number from the model's state
		...
		{
			$invoiceRecord->filename = $this->createPDF($record_table);
		}
		...
		// Attach the PDF invoice
                if($record_table == 'creditnotas') {
                   $mailer = AkeebasubsHelperEmail::getPreloadedMailer($sub, 'PLG_AKEEBASUBS_CREDITNOTAS_EMAIL');
                   $mailer->AddAttachment($path . $invoiceRecord->filename, 'creditnota.pdf', 'base64', 'application/pdf'); 
                } else {
